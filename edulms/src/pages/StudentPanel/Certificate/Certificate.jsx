...
const Certificate1 = ({ isAdmin }) => {
  ...
  const fetchStudentById = async (id) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/students/${id}/completed-courses`
      );
  ...
  const generateCertificateFromStudentData = (studentData) => {
      ...
      <body>
        <div class="certificate">
          ...
          <div class="content">
            ...
            <div class="course">${
              studentData.courseName || "Web Development Course"
            }</div>
            ...
            <div class="date-section">
              <p><strong>Date of Completion:</strong> ${
                studentData.completionDate
                  ? new Date(studentData.completionDate).toLocaleDateString()
                  : "N/A"
              }</p>
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `Certificate_${(studentData.name || "Student").replace(
      /\s+/g,
      "_"
    )}`;
    document.body.appendChild(link);
    ...
    alert(
      `Certificate downloaded successfully for ${
  ...
  const filteredStudents = students.filter(
    (student) =>
      (student.name?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
      (student.email?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
      (student.batchCode?.toLowerCase() || "").includes(
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  {selectedStudent && (
                    <div className="certificate-preview">
                      <div
                        className="text-center p-4 border border-3 border-warning rounded"
                        style={{
                          background:
                ...
                <div className="modal-footer">
                  ...
                  <button
                    ...
                    className="btn btn-success"
                    onClick={() =>
                      selectedStudent && downloadCertificate(selectedStudent)
                    }
                  >
                    <i className="fas fa-download me-2"></i>
                    Download Certificate
